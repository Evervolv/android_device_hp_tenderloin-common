...
TARGET_WLAN_MODULE_SRC := external/backports-wireless

KERNEL_WIFI_MODULES:
	$(MAKE) $(JOBS) $(MAKE_FLAGS) -C $(TARGET_WLAN_MODULE_SRC) KLIB=$(KERNEL_OUT) KLIB_BUILD=$(KERNEL_OUT) defconfig-ath6kl
	$(MAKE) $(JOBS) $(MAKE_FLAGS) -C $(TARGET_WLAN_MODULE_SRC) KLIB=$(KERNEL_OUT) KLIB_BUILD=$(KERNEL_OUT) ARCH=$(TARGET_ARCH) $(KERNEL_CROSS_COMPILE)
	mv `find $(TARGET_WLAN_MODULE_SRC) -name *.ko` $(KERNEL_MODULES_OUT)
	$(ARM_EABI_TOOLCHAIN)/arm-eabi-strip --strip-debug --strip-unneeded `find $(KERNEL_MODULES_OUT) -name *.ko`
	$(MAKE) -C $(TARGET_WLAN_MODULE_SRC) mrproper
